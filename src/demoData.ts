...
// ── Initial Demo Data ────────────────────────────────────────
import type { Task, CalendarEvent, ChatMessage, AppNotification } from './types';

export const INITIAL_NOTIFICATIONS: AppNotification[] = [
  { id: 'n1', text: "Octave a déplacé 'Refonte Dashboard UI' vers En cours", time: "Il y a 5 min", createdAt: Date.now() - 5 * 60000, unread: true },
  { id: 'n2', text: "Mourchid vous a assigné 'API d'intégration PostgreSQL'", time: "Il y a 1 h", createdAt: Date.now() - 60 * 60000, unread: true },
  { id: 'n3', text: "Nouveau post planifié avec succès pour LinkedIn", time: "Il y a 2 h", createdAt: Date.now() - 120 * 60000, unread: false },
];
...
export function saveNotifications(notifications: AppNotification[]): void {
  localStorage.setItem(LS_NOTIFICATIONS, JSON.stringify(notifications.slice(0, 30)));
}
...
export function relativeTime(fromMs: number): string {
  const diffMin = Math.max(0, Math.round((Date.now() - fromMs) / 60000));
...
// ── Réponses IA mockées (démo 100% offline, aucun appel réseau) ─
const AI_MOCK_RESPONSES: string[] = [
  "D'après le sprint en cours, 78% des tâches sont complétées. Le module Kanban montre 3 tâches à risque de retard sur 'Sprint Alpha'. Je recommande de prioriser l'API PostgreSQL avant la démo.",
  "J'ai analysé le calendrier éditorial : 3 posts sont programmés cette semaine sur LinkedIn, Instagram et WhatsApp. Le taux d'engagement moyen est en hausse de 12% ce mois-ci.",
  "L'équipe a une vélocité de 84 points ce sprint, au-dessus de la moyenne. Ezechiel et Mourchid portent la majorité des tâches à haute priorité. Souhaitez-vous que je rééquilibre la charge ?",
  "Résumé rapide : 8 tâches actives, 2 terminées cette semaine, aucun blocage critique signalé côté RH. Le module Analytics est à jour.",
  "Je peux générer un rapport d'activité PDF, planifier un post social ou réassigner une tâche. Dites-moi simplement ce dont vous avez besoin.",
];

export function getMockAIResponse(userMessage: string): string {
  const lower = userMessage.toLowerCase();
  if (lower.includes('rapport') || lower.includes('analytics')) {
    return "Voici un aperçu : vélocité 84 pts, 78% de complétion du sprint Alpha, engagement social +12%. Le rapport complet est disponible dans l'onglet Rapports & Analytics.";
  }
  if (lower.includes('tâche') || lower.includes('task') || lower.includes('kanban')) {
    return "Sur le Kanban actuel : 3 tâches 'À faire', 3 'En cours', 2 'Terminées'. La priorité la plus urgente est 'API d'intégration PostgreSQL' (échéance demain 12:00).";
  }
  if (lower.includes('équipe') || lower.includes('rh') || lower.includes('talent')) {
    return "L'équipe compte 4 profils actifs (Admin, Lead, Member, Observer côté client). Aucune alerte RH en attente cette semaine.";
  }
  const idx = Math.abs(userMessage.length + userMessage.charCodeAt(0)) % AI_MOCK_RESPONSES.length;
  return AI_MOCK_RESPONSES[idx];
}
